...
pm = pm.rename(columns={'grossProfitRatio':'profit_margin'})


pm_ttm = pd.DataFrame(income_statement_q['grossProfitRatio'][:1])
pm_ttm = pm_ttm.rename(columns={'grossProfitRatio':'profit_margin'})
...
fcf_growth_df = metrics_df[['fcf','fcf_growth','fcf_growth_3_yr','fcf_growth_5_yr']]

#historic averages

cols = ['roce', 'fcf_yield', 'cash_conversion', 'dividend_coverage', 'croci', 'roic' ,\
        'interest_coverage' , 'pe' , 'profit_margin' , 'operating_margin']
